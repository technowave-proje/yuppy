...
# --------------------------
cursor.execute("SELECT id, longitude, latitude, timestamp, weight FROM tempo_o2o2_data")
...
# --------------------------
# 6️⃣ Eyalet bazında ortalama O2O2
# --------------------------
grid_data['date'] = pd.to_datetime(grid_data['timestamp']).dt.date
state_avg_o2o2 = grid_data.groupby(['state_name', 'date'])['weight'].mean().reset_index()
state_avg_o2o2.rename(columns={'weight':'avg_o2o2'}, inplace=True)

# --------------------------
# 7️⃣ Sonucu CSV’ye kaydet
# --------------------------
state_avg_o2o2.to_csv("state_avg_o2o2.csv", index=False)
print("✅ Eyalet bazlı O2O2 verisi başarıyla oluşturuldu ve kaydedildi.")
'''
import mysql.connector
import pandas as pd
from geopy.distance import geodesic
...
cursor.execute("""
    ...
    AND TABLE_NAME = 'tempo_o2o2_data' 
    AND COLUMN_NAME = 'state_name'
""")
result = cursor.fetchone()

if result["cnt"] == 0:
    cursor.execute("ALTER TABLE tempo_o2o2_data ADD COLUMN state_name VARCHAR(100)")
    conn.commit()
...
# --------------------------
# 4) state_name olmayan tempo_o2o2_data verilerini çek
# --------------------------
cursor2 = conn.cursor(dictionary=True)
cursor2.execute("SELECT id, longitude, latitude FROM tempo_o2o2_data WHERE state_name IS NULL OR state_name = ''")
...
if to_update_df.empty:
    ...
    # --------------------------
    update_sql = "UPDATE tempo_o2o2_data SET state_name = %s WHERE id = %s"
