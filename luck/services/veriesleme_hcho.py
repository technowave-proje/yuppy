...
# --------------------------
cursor.execute("SELECT id, longitude, latitude, timestamp, weight FROM tempo_hcho_data")
...
# --------------------------
# 6️⃣ Eyalet bazında günlük ortalama HCHO
# --------------------------
grid_data['date'] = pd.to_datetime(grid_data['timestamp']).dt.date
state_avg_hcho = grid_data.groupby(['state_name', 'date'])['weight'].mean().reset_index()
state_avg_hcho.rename(columns={'weight':'avg_hcho'}, inplace=True)

# --------------------------
# 7️⃣ Sonucu CSV’ye kaydet
# --------------------------
state_avg_hcho.to_csv("state_avg_hcho.csv", index=False)
print("✅ Eyalet bazlı HCHO verisi başarıyla oluşturuldu ve kaydedildi.")
'''
...
cursor.execute("""
    ...
    AND TABLE_NAME = 'tempo_hcho_data' 
    AND COLUMN_NAME = 'state_name'
""")
result = cursor.fetchone()

if result["cnt"] == 0:
    cursor.execute("ALTER TABLE tempo_hcho_data ADD COLUMN state_name VARCHAR(100)")
    conn.commit()
...
# --------------------------
# 4) state_name olmayan tempo_hcho_data verilerini çek
# --------------------------
cursor2 = conn.cursor(dictionary=True)
cursor2.execute("SELECT id, longitude, latitude FROM tempo_hcho_data WHERE state_name IS NULL OR state_name = ''")
...
if to_update_df.empty:
    ...
    # --------------------------
    update_sql = "UPDATE tempo_hcho_data SET state_name = %s WHERE id = %s"
